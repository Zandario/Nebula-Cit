...
/obj/machinery/power/apc/ui_interact(mob/user, ui_key = "main", datum/nanoui/ui, force_open = TRUE)
	if(!user)
		return
	...
	if (!ui)
		...
		ui = new(user, src, ui_key, "apc", "[area? area.proper_name : "ERROR"] - APC", 520, data["siliconUser"] ? 465 : 440)
