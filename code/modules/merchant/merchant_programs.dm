...
/datum/nano_module/program/merchant/ui_interact(mob/user, ui_key = "main", datum/nanoui/ui, force_open = TRUE, datum/topic_state/state = global.default_topic_state)
	...
	if (!ui)
		ui = new(user, src, ui_key, "merchant", "Merchant List", 575, 700, state = state)
