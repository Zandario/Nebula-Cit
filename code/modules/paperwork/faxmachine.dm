...
/obj/machinery/faxmachine/ui_interact(mob/user, ui_key, datum/nanoui/ui, force_open, datum/nanoui/master_ui, datum/topic_state/state)
	...
	if (!ui)
		ui = new(user, src, ui_key, "fax_machine.jst", name, 640, 480)
		ui.add_template("net_shared",                 "network_shared.jst") //Shared network UI stuff
		ui.add_template("stock_parts_printer_shared", "stock_parts_printer.jst")
