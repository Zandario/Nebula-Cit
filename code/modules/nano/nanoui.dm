 ...
 */
/datum/nanoui/New(nuser, nsrc_object, nui_key, ntemplate_filename, ntitle, nwidth, nheight, datum/nref, datum/nanoui/master_ui, datum/topic_state/state = global.default_topic_state)
	user = nuser
 ...
 *
 * @param push_update int (bool) Push an update to the ui to update it's status. This is set to FALSE/0 if an update is going to be pushed anyway (to avoid unnessary updates)
 *
 * @return TRUE/1 if closed, null otherwise.
 */
 ...
 *
 * @param state int (bool) Set auto update to TRUE or FALSE (1/0)
 *
 * @return nothing
 */
