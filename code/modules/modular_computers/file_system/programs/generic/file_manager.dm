...
/datum/computer_file/program/filemanager/ui_interact(mob/user, ui_key = "main", datum/nanoui/ui, force_open = TRUE, datum/topic_state/state = global.default_topic_state)
	. = ..()
	if(!.)
		return
	...
	if (!ui)
		ui = new(user, src, ui_key, "file_manager", "OS File Manager", 900, 700, state = state)
