...
/datum/nano_module/program/comm/ui_interact(mob/user, ui_key = "main", datum/nanoui/ui, force_open = TRUE, datum/topic_state/state = global.default_topic_state)
	...
	if(!ui)
		ui = new(user, src, ui_key, "communication", name, 550, 420, state = state)
