...
/datum/nano_module/atmos_control/ui_interact(mob/user, ui_key = "main", datum/nanoui/ui, force_open = TRUE, var/master_ui = null, datum/topic_state/state = global.default_topic_state)
	...
	if(!ui)
		ui = new(user, src, ui_key, "atmos_control", src.name, 625, 625, state = state)
