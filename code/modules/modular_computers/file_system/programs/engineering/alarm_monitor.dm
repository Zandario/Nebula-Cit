...
/datum/nano_module/alarm_monitor/ui_interact(mob/user, ui_key = "main", datum/nanoui/ui, force_open = TRUE, datum/topic_state/state = global.default_topic_state)
	...
	if (!ui)
		ui = new(user, src, ui_key, "alarm_monitor", "Alarm Monitoring Console", 800, 800, state = state)
