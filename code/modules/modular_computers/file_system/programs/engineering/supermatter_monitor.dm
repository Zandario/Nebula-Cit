...
/datum/nano_module/program/supermatter_monitor/ui_interact(mob/user, ui_key = "main", datum/nanoui/ui, force_open = TRUE, datum/topic_state/state = global.default_topic_state)
	...
	if (!ui)
		ui = new(user, src, ui_key, "supermatter_monitor", "Supermatter Monitoring", 600, 400, state = state)
