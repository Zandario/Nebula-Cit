...
/obj/item/aicard/ui_interact(mob/user, ui_key = "main", datum/nanoui/ui, force_open = TRUE, datum/topic_state/state = global.inventory_topic_state)
	var/data[0]
	...
	if (!ui)
		ui = new(user, src, ui_key, "aicard", "[name]", 600, 400, state = state)
