 ...
 * If you don't want your atom's icon to smooth with anything but atoms of the same type, set the list 'atom_can_smooth_with' to null;
 * Otherwise, put all the smoothing groups you want the atom icon to smooth with in 'atom_can_smooth_with', including the group of the atom itself.
 * Smoothing groups are just shared flags between objects. If one of the 'atom_can_smooth_with' of A matches one of the `smoothing_groups` of B, then A will smooth with B.
 *
 ...
 * to add the 'SMOOTHING_FLAG_DIAGONAL_CORNERS' flag to the atom's smoothing_flags var (in addition to either SMOOTHING_FLAG_TRUE or SMOOTHING_FLAG_MORE).
 *
...
#define IS_SMOOTH(A) (A.smoothing_flags & (SMOOTHING_FLAG_CORNERS|SMOOTHING_FLAG_BITMASK|SMOOTHING_FLAG_CUSTOM))

#define QUEUE_SMOOTH(thing_to_queue) \
	if(IS_SMOOTH(thing_to_queue)) { \
...
#define SET_ADJ_IN_DIR(source, junction, direction, direction_flag) \
	do { \
		...
		if(!neighbor) { \
			if(source.smoothing_flags & SMOOTHING_FLAG_BORDER) { \
				junction |= direction_flag; \
			}; \
		}; \
		else { \
			if(!isnull(neighbor.smoothing_groups)) { \
				for(var/target in source.atom_can_smooth_with) { \
					if(!(source.atom_can_smooth_with[target] & neighbor.smoothing_groups[target])) { \
						continue; \
					}; \
					junction |= direction_flag; \
					break; \
			...
			if(!(junction & direction_flag) && source.smoothing_flags & SMOOTHING_FLAG_OBJ) { \
				for(var/obj/thing in neighbor) { \
					if(!thing.anchored || isnull(thing.smoothing_groups)) { \
						continue; \
					}; \
					for(var/target in source.atom_can_smooth_with) { \
						if(!(source.atom_can_smooth_with[target] & thing.smoothing_groups[target])) { \
							continue; \
						}; \
						junction |= direction_flag; \
						break; \
...
/atom/proc/calculate_adjacencies()
	...
	for(var/direction in cardinal)
		switch(find_type_in_direction(direction))
			if(NULLTURF_BORDER)
				if((smoothing_flags & SMOOTHING_FLAG_BORDER))
...
/obj/calculate_adjacencies()
	if((obj_flags & OBJ_FLAG_ANCHORABLE && !anchored))
		return 0
	return ..()
...
/atom/proc/smooth_icon()
	smoothing_flags &= ~SMOOTHING_FLAG_QUEUED
	atom_flags |= ATOM_FLAG_HTML_USE_INITAL_ICON
	if (!z)
		CRASH("[type] called smooth_icon() without being on a z-level")
	if(smoothing_flags & SMOOTHING_FLAG_CORNERS)
		if(smoothing_flags & SMOOTHING_FLAG_DIAGONAL_CORNERS)
			corners_diagonal_smooth(calculate_adjacencies())
		else
			corners_cardinal_smooth(calculate_adjacencies())
	else if(smoothing_flags & SMOOTHING_FLAG_BITMASK)
		bitmask_smooth()
	else if(smoothing_flags & SMOOTHING_FLAG_CUSTOM)
		custom_smooth(calculate_adjacencies()) //! Snowflake smoothing.
	else
...
/atom/proc/find_type_in_direction(direction)
	...
	if(isnull(atom_can_smooth_with)) //special case in which it will only smooth with itself
		if(isturf(src))
	...
	if(!isnull(target_turf.smoothing_groups))
		for(var/target in atom_can_smooth_with)
			if(!(atom_can_smooth_with[target] & target_turf.smoothing_groups[target]))
				continue
			return ADJ_FOUND

	if(smoothing_flags & SMOOTHING_FLAG_OBJ)
		for(var/am in target_turf)
			var/atom/movable/thing = am
			if(!thing.anchored || isnull(thing.smoothing_groups))
				continue
			for(var/target in atom_can_smooth_with)
				if(!(atom_can_smooth_with[target] & thing.smoothing_groups[target]))
					continue
				return ADJ_FOUND
